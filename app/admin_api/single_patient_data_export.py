    ...
    def iter_data_rows(self, patient_id):
        for visit in patient_visits(patient_id):
            ...
            for event in events_by_visit(visit.id):
                ...
                elif event.event_type == 'Common Problems':
                    write_common_problems_event(row, event)
                elif event.event_type == 'Mental Health':
                    write_mental_health_event(row, event)
                elif event.event_type == 'Interventions':
                    write_interventions_event(row, event)
