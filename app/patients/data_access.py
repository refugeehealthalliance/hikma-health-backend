...
def add_patient(patient: Patient):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute('INSERT INTO patients (id, given_name, surname, date_of_birth, sex, country, hometown, phone, number, edited_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',
                        [patient.id,
...
def all_patient_data():
    query = """
    SELECT id, given_name, surname, date_of_birth, sex, country, hometown, phone, number, edited_at FROM patients ORDER BY edited_at DESC LIMIT 25
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(query, [])
...
def search_patients(given_name: str, surname: str, country: str, hometown: str):
    ...
    query = f"SELECT id, given_name, surname, date_of_birth, sex, country, hometown, phone, number, edited_at FROM patients WHERE {where_clause};"
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(query, params)
            yield from cur
...
def patient_from_id(patient_id):
    query = """
    SELECT given_name, surname, date_of_birth, sex, country, hometown, phone, number, edited_at FROM patients WHERE id = %s
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            if row is None:
                return None
            given_name, surname, date_of_birth, sex, country, hometown, phone, number, edited_at = row
            return Patient(
                id=patient_id,
